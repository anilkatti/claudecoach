...
Reimplementation of the reference pipeline:
...
STAGE 1 — collection. The reference uploader writes TSV/numstat files that Rails
...
sessions.jsonl. Rationale: The reference uploader bounds git collection to the
...
PORTED QUIRKS (kept on purpose — they are what the reference actually computes):
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Commit grouping + episode linking (CommitGrouper/EpisodeLinker reimplementation).")

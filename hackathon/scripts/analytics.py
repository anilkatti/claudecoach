#!/usr/bin/env python3
"""
analytics.py — Python reimplementation of the reference client-side UPLOAD-ONLY
analytics. These metrics NEVER feed the local score — they are a transparency
report of what the reference server receives alongside the condensed transcripts.
...
DOCUMENTED SUBSTITUTION — /git_metrics.txt:
  The reference container reads a pre-packaged /git_metrics.txt
...
UPLOAD_ONLY_NOTE = (
    ...
    "metrics the reference client computes and uploads. They never feed the local "
    "score.")
...
def render_markdown(report):
    lines = ["# Upload Analytics (local transparency report)", ""]
...
def main():
    ap = argparse.ArgumentParser(
        description="Upload-only analytics (local transparency report)")

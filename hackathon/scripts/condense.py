...
Reimplementation of the reference client-side condensing path (TranscriptChunker +
...
the scorer must see the SAME condensed text the reference pipeline produces, not the raw
...
# to Claude Code canonical entries BEFORE condensing, exactly as the reference pipeline does, so
...
def _codex_convert(payload):
    ...
    if ptype == "custom_tool_call":
        ...
        # top-level "input" field. NOT in the archived reference normalizer (its
    ...
    if ptype == "reasoning":
        ...
        # the canonical stream like the reference pipeline's.
        ...
        if role == "user":
            ...
            for blk in blocks(msg.get("content")):
                ...
                if t == "text":
                    ...
                    # by a few bytes from the reference pipeline's own count. Representative, not exact.

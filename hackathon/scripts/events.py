...
Reimplementation of the reference client-side transcript event extraction:
...
def extract_model_usage(entries, is_subagent):
    """Additive per-session accounting (NOT from the reference — local report data).
...
def extract_prompt_stats(user_messages):
    """Additive prompt stats for the report (NOT from the reference): median words,
    count of prompts <= SHORT_PROMPT_MAX_WORDS (the existing reference constant),
...
def extract_session(path):
    ...
    # Subagent detection: the reference knows from the discovery layout (subagents/
